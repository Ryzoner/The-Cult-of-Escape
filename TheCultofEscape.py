...
class Player:
    ...
    def set_skin(self, skin_name: str, skin_type: str = ''):
        #TODO Добавить словарь со всеми скинами
    ...
    def draw_jump(self):
        ...
        neg - если Парабола увеличивается neg = -1 иначе 1
        """
        ...
        if not self.state['sit']:
            ...
            if direction == 'right':
                ...
                if not self.state['flip']:
                    self.flip()
                if self.is_in_window([self.skin.rect.x - i, self.skn.rect.t]):
                    self.skin.rect.x -= i
...
class Game:
    ...
    def start(self):
        self.player: Player = Player([0, 400 - 32], 2, self.settings['skin'])
